...
async def list_files(db: Session = Depends(get_db)):
    ...
    try:
        # Query all parsed contents with their associated files
        logger.info("Executing database query...")
        parsed_contents = db.query(ParsedContent).join(UploadedFile).all()
        ...
        files = [
            {
                ...
                "text": content.parsed_text,
                "metadata": content.content_metadata,
                "parsed_at": content.parse_time.isoformat() if content.parse_time else None,
                "updated_at": content.last_updated.isoformat() if content.last_updated else None,
        ...
        logger.info(f"Preparing response with {len(files)} files")
